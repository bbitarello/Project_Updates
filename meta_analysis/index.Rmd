...
dt=lapply(c("anc_specific", "1000g_all"), function(x) fread(paste0('ldpred_table_radius1000_boots_1000_ldpanel_', x, '.txt'))[, LD_panel:=x])
dt<-do.call(rbind, dt)
dt[, Summary_Stats:=gsub("PRS_", "", Summary_Stats)]
dt<-dt[Summary_Stats!=c("GIANT2","GIANT0")]
...
theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12), axis.title.y=element_text(size=rel(1.4)), axis.title.x=element_text("Variance Explained %"))

ss_lab<-levels((dt %>% mutate(Summary_Stats=fct_reorder(Summary_Stats, N)))$Summary_Stats)
#ggplotly(p2)
...
shapes<-c(16,17,18,6,15)
setnames(dt[[1]], c('R2', 'y'), c('Anc_Specific_R2','1000g_all_R2'))
p3<-dt[[1]] %>% 
        dplyr::mutate(Label2=fct_reorder(Label2,pEUR)) %>% mutate(Label=fct_reorder(Label, N)) %>%
        #filter(!Summary_Stats %in% c("META_ALL", "META_ALL2", "META_AFR2", "META_EUR")) %>% 
        ggplot(aes(x=Anc_Specific_R2, y=`1000g_all_R2`)) +
geom_point(aes(shape=Test, colour=Label), size=2.5, alpha=0.8, fill='white', show.legend=T) +
        geom_errorbarh(aes(xmin=Anc_Specific_R2+SE, xmax=Anc_Specific_R2-SE, colour=Label), width=0.03, linetype = "dotted", alpha=0.5)+
        geom_errorbar(aes(ymin=`1000g_all_R2`+SE.y, ymax=`1000g_all_R2`-SE.y, colour=Label), width=0.03, linetype = "dotted", alpha=0.5)+
...
subplot(p2g, p3g, nrows = 2, margin = 0.03, shareX=F, shareY=F, titleX=T, titleY=T)
...
```{r, echo=F}
set.seed(418)
cols2<-unname(sample(unlist(wesanderson::wes_palettes),4))
res1<-readRDS('resPRS_multi.RDs')
nams<-names(res1)
sets<-names(res1[[1]])
sets<-sets[grep('GIANT2',sets, invert=T)]
alphas<-names(res1[[1]][[1]])
tests<-names(res1[[1]][[1]][[1]])

test<-lapply(nams, function(i) lapply(sets, function(j) lapply(alphas, function(a) data.table(Set=j, Test_Data=i, Alpha=as.numeric(gsub("alpha", "",a)), PRS1=res1[[i]][[j]][[a]]$PRS1, PRS2=res1[[i]][[j]][[a]]$PRS2, PRS2b=res1[[i]][[j]][[a]]$PRS2b))))

RES<-rbind(do.call(rbind, lapply(test[[1]], function(x) do.call(rbind, x))),
do.call(rbind, lapply(test[[2]], function(x) do.call(rbind, x))),
do.call(rbind, lapply(test[[3]], function(x) do.call(rbind, x))))

setDT(RES)
RES<-melt(RES, id.vars=c("Set","Test_Data", "Alpha"))

RES[, Label:=ifelse(Test_Data=='HRS_AFR', addline_format(paste0(Test_Data, " (", 2241, ")")),ifelse(Test_Data=='PMBB_AFR',addline_format(paste0(Test_Data, " (", 8673, ")")), addline_format(paste0(Test_Data, " (", 1467, ")"))))]
RES$Label<-factor(RES$Label)
label1<-levels(RES$Label)

expnd<-rbind(data.table(Set=c("UKBB", "META_NEA"), Test_Data="HRS_AFR",  Alpha=c(0,1), value=c(5.54,4.72),PRS="PRS", Label="HRS_AFR\n(2241)"),
             data.table(Set=c("META_EUR", "META_NEA"), Test_Data="HRS_AFR", Alpha=c(0,1), value=c(6.12,4.72),PRS="PRS", Label="HRS_AFR\n(2241)"),
             data.table(Set=c("META_EUR", "META_ALL2"), Test_Data="HRS_AFR", Alpha=c(0,1), value=c(6.12,8.29),PRS="PRS", Label="HRS_AFR\n(2241)"),
             data.table(Set=c("UKBB", "META_ALL2"), Test_Data="HRS_AFR", Alpha=c(0,1), value=c(5.54,8.29),PRS="PRS", Label="HRS_AFR\n(2241)"),
             data.table(Set=c("UKBB","META_NEA"), Test_Data="PMBB_AFR",  Alpha=c(0,1),value=c(4.97,5.42), PRS="PRS", Label="PMBB_AFR\n(8673)"),
             data.table(Set=c("META_EUR", "META_NEA"), Test_Data="PMBB_AFR", Alpha=c(0,1), value=c(5.49,5.42),PRS="PRS", Label="PMBB_AFR\n(8673)"),
             data.table(Set=c("META_EUR", "META_ALL2"), Test_Data="PMBB_AFR", Alpha=c(0,1), value=c(5.49,8.07),PRS="PRS", Label="PMBB_AFR\n(8673)"),
             data.table(Set=c("UKBB", "META_ALL2"), Test_Data="PMBB_AFR", Alpha=c(0,1), value=c(4.97,8.07),PRS="PRS", Label="PMBB_AFR\n(8673)"),
             data.table(Set=c("UKBB","BBJ"), Test_Data="UKB_CHI",  Alpha=c(0,1), value=c(10.65,7.81),PRS="PRS", Label="UKB_CHI\n(1467)"),
             data.table(Set=c("META_EUR", "BBJ"), Test_Data="UKB_CHI", Alpha=c(0,1), value=c(10.75,7.81), PRS="PRS", Label="UKB_CHI\n(1467)"),
             data.table(Set=c("META_EUR", "META_ALL2"), Test_Data="UKB_CHI", Alpha=c(0,1), value=c(10.75,12.25), PRS="PRS", Label="UKB_CHI\n(1467)"))

expnd[,value:=round(value, 2)]
...
setnames(prs3,'Partial_Rsq','value')
prs3[, variable:="PRS3"][, Test_Data:="PMBB_AFR"][, Label:="PMBB_AFR\n(8673)"]
prs3<-prs3[,.(Set, Test_Data, Alpha, variable, value, Label)]
RES2<-rbind(RES, prs3)
setnames(RES2, 'variable', 'PRS')        
RES2$Set<-gsub("PRS_", "",RES2$Set)
RES2[, value:=round(value, 2)]
#pD<-ggplot(RES2[Test_Data=="HRS_AFR" & PRS %in% c("PRS1", "PRS2")][Set %in% c("UKBB--META_NEA", "META_EUR--META_NEA", "UKBB--META_ALL2", "META_EUR--META_ALL2")]) +
pD<-ggplot(RES2[Test_Data=="HRS_AFR" & PRS %in% c("PRS1", "PRS2")][Set %in% c("UKBB--META_NEA", "META_EUR--META_NEA")])+
geom_line(aes(x=Alpha, y=value, color=Set, linetype=PRS), size=0.6, show.legend=F) + geom_point(data=expnd[Test_Data=="HRS_AFR"][c(1:3)],aes(x=Alpha, y=value, color=Set), size=2.5, col=cols[c(9,5,4)], alpha=c(0.8, 0.8, 0.8)) +
         geom_text(data=expnd[Test_Data=="HRS_AFR"][Set=="META_ALL2"], aes(x=Alpha, y=value, label=Set), nudge_x=-0.1, nudge_y=0.1, check_overlap=T, size=3)+
...
geom_hline(yintercept=expnd[Test_Data=="HRS_AFR"][Set=="META_ALL2"]$value,  col=cols[10])+
...
#
#
pD<-pD+ annotate("text", x = 0.05, y = 8.5, label = "HRS_AFR", size=4)
#
#pE<-ggplot(RES2[Test_Data=="PMBB_AFR"& PRS %in% c("PRS1", "PRS2")][Set %in% c("UKBB--META_NEA", "META_EUR--META_NEA", "UKBB--META_ALL2", "META_EUR--META_ALL2")]) +
pE<-ggplot(RES2[Test_Data=="PMBB_AFR" & PRS %in% c("PRS1", "PRS2")][Set %in% c("UKBB--META_NEA", "META_EUR--META_NEA")])+
geom_line(aes(x=Alpha, y=value, color=Set, linetype=PRS), size=0.6, show.legend=T) + 
        geom_point(data=expnd[Test_Data=="PMBB_AFR"][c(1:3)],aes(x=Alpha, y=value), size=2, col=cols[c(9,5,4)], alpha=c(0.8, 0.8, 0.8)) +
        geom_text(data=expnd[Test_Data=="PMBB_AFR"][Set=="META_ALL2"], aes(x=Alpha, y=value, label=Set), nudge_x=-0.1, nudge_y=0.1, check_overlap=T, size=3)+
scale_color_manual(name="",values = cols2) + scale_y_continuous(breaks = seq(0, 10, by = 1)) +
theme_bw() + 
labs(x="Alpha",y="Variance explained (%)", title="", legend="") + 
geom_hline(yintercept=expnd[Test_Data=="PMBB_AFR"][Set=="META_ALL2"]$value,  col=cols[10])+
        theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12),axis.title.y=element_text(size=rel(1.4)), legend.title = element_blank())
#
pE<-pE+ annotate("text", x = 0.05, y = 8.5, label = "PMBB_AFR", size=4)
#

set.seed(419)
cols2<-unname(sample(unlist(wesanderson::wes_palettes),4))
#pF<-ggplot(RES2[Test_Data=="UKB_CHI" & PRS %in% c("PRS1", "PRS2")][Set %in% c("UKBB--BBJ", "META_EUR--BBJ", "UKBB--META_ALL2","META_EUR--META_ALL2")]) +
pF<-ggplot(RES2[Test_Data=="UKB_CHI" & PRS %in% c("PRS1", "PRS2")][Set %in% c("UKBB--BBJ", "META_EUR--BBJ")])+
geom_line(aes(x=Alpha, y=value, color=Set, linetype=PRS), size=0.6, show.legend=T) + 
  geom_text(data=expnd[Test_Data=="UKB_CHI"][Set=="META_ALL2"], aes(x=Alpha, y=value, label=Set), nudge_x=-0.1, nudge_y=0.1, check_overlap=T, size=3)+
scale_color_manual(name="",values = cols2) + scale_y_continuous(breaks = seq(0, 10, by = 1)) +
theme_bw() + 
labs(x="Alpha",y="Variance explained (%)", title="", legend="") + 
geom_hline(yintercept=expnd[Test_Data=="UKB_CHI"][Set=="META_ALL2"]$value,  col=cols[10])+
        theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12),axis.title.y=element_text(size=rel(1.4)), legend.title = element_blank())
#
pF<-pF+ annotate("text", x = 0.05, y = 14, label = "UKB_CHI", size=4)
```
...
subplot(style(ggplotly(pD), showlegend=F), ggplotly(pE), nrows=2, shareX=T, shareY=T)
```
...
```{r, chunk='Fig6', out.height = "1260px",out.width="1260px",echo=FALSE, eval=T, fig.cap="Fig 6: PRS for height for 2,503 individuals from the 1000 Genomes Project. PRS values are scaled around the mean and standard deviations of PRS for the EUR population, for each set of summary statistics.LD panel: anc specific."}
set.seed(125)
cols3<-unname(sample(unlist(wesanderson::wes_palettes),5))
