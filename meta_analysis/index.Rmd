...
```{r, chunk='Fig2', out.height = "360px",out.width="860px",echo=FALSE, eval=T, fig.cap="Fig 2: Proportion of variance explained. PRS calculated using linear combinations of PRS weights from LDpred-inf (PRS1, PRS2) and using local ancestry PRS (PRS3)."}
set.seed(4194)
cols2<-unname(sample(unlist(wesanderson::wes_palettes),15))
...
pA<-res3[Test_Data %in% c('PMBB_AFR', 'HRS_AFR', 'UKB_CHI')] %>% 
        filter(Set %in% unique(do.call(rbind, (res3 %>% filter(Test_Data %in% c('PMBB_AFR', 'HRS_AFR', 'UKB_CHI')) %>% group_by(Test_Data) %>% mutate(Test=Test_Data) %>% arrange(-value) %>% group_map(~head(.x, 5L))))$Set)) %>% mutate(Set=fct_reorder(Set, value)) %>% ggplot(aes(x=Alpha, y=value, color=Set)) + facet_wrap(~Test_Data)+geom_line() + 
        scale_color_manual(name='Summary Statistics',values = cols2[1:12])  + scale_y_continuous(breaks = seq(0, 20, by = 2)) + theme_bw()
#
...
pB<-res4 %>% 
        filter(Set %in% unique(do.call(rbind, (res4 %>% group_by(Test_Data) %>% mutate(Test=Test_Data) %>% arrange(-value) %>% group_map(~head(.x, 5L))))$Set)) %>% mutate(Set=fct_reorder(Set, value)) %>% ggplot(aes(x=Alpha, y=value, color=Set)) + facet_wrap(~Test_Data)+geom_line() + 
        scale_color_manual(name='Summary Statistics',values = cols2[1:12])  + scale_y_continuous(breaks = seq(0, 20, by = 2)) + theme_bw()
...
#pC<-cowplot::plot_grid(pA,pB, labels=c("PRS1", "PRS2"), nrow=2)
pC<-ggplot(res5) + geom_line(aes(x=Alpha, y=value, color=Set, linetype=PRS), size=0.9) + facet_wrap(~Test_Data, dir="h") +scale_color_manual(name='Summary Statistics',values = cols2[1:12])  + scale_y_continuous(breaks = seq(0, 10, by = 2)) + theme_bw() + labs(x="",y="Variance explained (%)", title="") + theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12), axis.title.y=element_text(size=rel(1.4)))

dt2<-dt[Summary_Stats %in% c("META_EUR", "UKBB", "META_NEA")][Test %in% c("HRS_AFR", "PMBB_AFR")][,.(Summary_Stats, Test, R2)] %>% mutate(Alpha=ifelse(Summary_Stats=="UKBB", 0, ifelse(Summary_Stats=="META_EUR", 0, 1)))%>% mutate(PRS="PRS", Label=NA) %>% as.data.table
setnames(dt2,c("Summary_Stats","Test", "R2"), c("Set", "Test_Data", "value"))


ggplotly(pC)

expnd<-rbind(expand.grid(Set="UKBB--META_NEA", Test_Data="HRS_AFR", value=c(5.54,5.28), Alpha=c(0,1), PRS="PRS", Label="HRS_AFR\n(2241)"), expand.grid(Set="META_EUR--META_NEA", Test_Data="HRS_AFR", value=c(6.13,5.28), Alpha=c(0,1), PRS="PRS", Label="HRS_AFR\n(2241)"),expand.grid(Set="UKBB--META_NEA", Test_Data="PMBB_AFR", value=c(4.97,5.58), Alpha=c(0,1), PRS="PRS", Label="PMBB_AFR\n(8673)"), expand.grid(Set="META_EUR--META_NEA", Test_Data="PMBB_AFR", value=c(5.65,5.58), Alpha=c(0,1), PRS="PRS", Label="PMBB_AFR\n(8673)"))


ggplot(res5[PRS %in% c("PRS1", "PRS2")]) + 
geom_line(aes(x=Alpha, y=value, color=Set, linetype=PRS), size=0.9) + 
scale_color_manual(name='Summary Statistics',values = cols2[1:12]) + scale_y_continuous(breaks = seq(0, 10, by = 2)) + 
facet_wrap(~Test_Data, dir="h") + 
theme_bw() + 
labs(x="",y="Variance explained (%)", title="") + theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12), axis.title.y=element_text(size=rel(1.4)))
+
annotate(x=0, y=)
