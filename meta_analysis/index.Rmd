...
theme_bw() + 
labs(x="Alpha",y="Variance explained (%)", title="", legend="") + theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12), axis.title.y=element_text(size=rel(1.4)), legend.title = element_blank(), plot.title = element_text("PMBB_AFR"))+
...
```{r, chunk='Fig3',out.height = "560px",out.width="960px",echo=FALSE, eval=T, fig.cap="Fig 2: Proportion of variance explained. PRS calculated using linear combinations of PRS weights from LDpred-inf (PRS1, PRS2) and using local ancestry PRS (PRS3).HRS_AFR and PMBB_AFR"}
subplot(style(ggplotly(pD), showlegend=F), ggplotly(pE))
```
...
g1000$Label<-factor(g1000$Label,levels=levels(factor(g1000$Label))[c(2,7,1,6,9,3,4,5,8,10)])
...
```{r, chunk='Fig4', out.height = "1260px",out.width="960px",echo=FALSE, eval=T, fig.cap="Fig 4: PRS for height for 2,503 individuals from the 1000 Genomes Project. PRS values are scaled around the mean and standard deviations of PRS for the EUR population, for each set of summary statistics."}
