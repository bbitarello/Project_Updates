...
g1000$Label<-factor(g1000$Label, levels=c("GIANT", "UKBB",  "BBJ","PAGE", "UKBB+GIANT", "META_AFR", "META_AFR+PAGE", "META_AFR+\nPAGE+BBJ", "UKBB+BBJ+\nMETA_AFR+PAGE", "UKBB+GIANT+\nBBJ+META_AFR+PAGE"))
g1000<-g1000[order(Label)]
g1000[,Label2:=ifelse(Label %in% c("UKBB", "GIANT", "BBJ", "PAGE"), "Single Study", ifelse(Label %in% c("UKBB+GIANT\n, META_AFR"), "Single ancestry", "Multiple"))]


p1000_A<-ggplot(g1000, aes(x=PRS_scaled, fill=super_pop))+ 
geom_density(alpha=0.6) + labs(y="Density",x="Scaled Polygenic Score", title="")+
        facet_wrap(~Label, labeller = label_wrap_gen(width=1), nrow=3, strip.position="right") +
theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12), axis.title.y=element_text(size=rel(1.4)), legend.direction="horizontal", plot.margin = margin(3, 3, 1, 1, "cm")) +
