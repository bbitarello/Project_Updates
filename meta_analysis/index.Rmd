...
```{r, chunk='Fig 1', out.height = "760px",out.width="1260px",echo=FALSE, eval=T, fig.cap="Fig 1: Proportion of variance explained. LDpred-Inf with sumamry statistics from UKBB, BBJ, meta-AFR, and meta-ALL. Points are empirical estimtes, and error bars are bootrstrap standard errors (n=5,000)."}
...
#ggplotly(p2)
p2<-p2+guides(shape=FALSE, color=FALSE)
p2g<-ggplotly(p2)
for(p2 in 1:length(p2g[['x']][['data']])){p2g[['x']][['data']][[i]]$showlegend<-F}
```

```{r, chunk='Fig 2', out.height = "760px",out.width="910px",echo=FALSE, eval=T, fig.cap="Fig 2: Proportion of variance explained when using 1000G (all) as LD reference panel vs. using ancestry-matching LD panels."}
dt=lapply(c("anc_specific", "1000g_all"), function(x) fread(paste0('ldpred_table_radius1000_boots_5000_ldpanel_', x, '.txt'))[, LD_panel:=x])
dt<-do.call(rbind, dt)
dt[, Summary_Stats:=gsub("PRS_", "", Summary_Stats)]
dt[,Label:=paste0(Label, ";", N)]
