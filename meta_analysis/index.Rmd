...
```{r, chunk='Fig 1', out.height = "760px",out.width="960px",echo=FALSE, eval=T, fig.cap="Fig 1: Proportion of variance explained. LDpred-Inf with sumamry statistics from UKBB, BBJ, meta-AFR, and meta-ALL. Points are empirical estimtes, and error bars are bootrstrap standard errors (n=5,000)."}
...
geom_errorbar(aes(ymin=R2+SE, ymax=R2-SE), width=0.05, linetype = "dotted",position=position_dodge(width=0.8)) +        facet_wrap(~LD_panel, nrow=2) +
...
```{r, chunk='Fig 2', out.height = "460px",out.width="710px",echo=FALSE, eval=T, fig.cap="Fig 2: Proportion of variance explained when using 1000G (all) as LD reference panel vs. using ancestry-matching LD panels."}
dt=lapply(c("anc_specific", "1000g_all"), function(x) fread(paste0('ldpred_table_radius1000_boots_5000_ldpanel_', x, '.txt'))[, LD_panel:=x])
dt<-do.call(rbind, dt)
dt[, Summary_Stats:=gsub("PRS_", "", Summary_Stats)]
dt[,Label:=paste0(Label, ";", N)]
...
p3<-dt[[1]] %>% filter(!Summary_Stats %in% c("META_ALL", "META_ALL2", "META_AFR2")) %>% ggplot(aes(x=R2, y=y)) +
        theme_bw()+
geom_point(aes(shape=Test, colour=Label), size=5, alpha=0.9, fill='white', show.legend=T) +
scale_color_manual(name='',values = cols[c(1,2,4,5,6,7,9)]) + 
        ...
        geom_abline(intercept=0, slope=1, col='darkgray', lty=2, pwd=2) +
...
p3+geom_text(aes(x=15,y=17),label="x=y")
...
res5<-bind_rows(res3[Test_Data %in% c('PMBB_AFR', 'HRS_AFR', 'UKB_CHI')] %>% mutate(Set=fct_reorder(Set, value)) %>% mutate(PRS="PRS1"), res4[Test_Data %in% c('PMBB_AFR', 'HRS_AFR', 'UKB_CHI')] %>%  mutate(Set=fct_reorder(Set, value)) %>% mutate(PRS="PRS2")) %>% filter(Set %in% c("UKBB--META_NEA", "META_EUR--META_NEA")) %>% as.data.table
...
expnd<-rbind(data.table(Set=c("UKBB", "META_NEA"), Test_Data="HRS_AFR", value=c(5.54,4.72), Alpha=c(0,1), PRS="PRS", Label="HRS_AFR\n(2241)"), data.table(Set=c("META_EUR", "META_NEA"), Test_Data="HRS_AFR", value=c(6.12,4.72), Alpha=c(0,1), PRS="PRS", Label="HRS_AFR\n(2241)"),data.table(Set=c("UKBB","META_NEA"), Test_Data="PMBB_AFR", value=c(4.97,5.42), Alpha=c(0,1), PRS="PRS", Label="PMBB_AFR\n(8673)"), data.table(Set=c("META_EUR", "META_NEA"), Test_Data="PMBB_AFR", value=c(5.49,5.42), Alpha=c(0,1), PRS="PRS", Label="PMBB_AFR\n(8673)")) 
...
```{r, chunk='Fig3',out.height = "560px",out.width="960px",echo=FALSE, eval=T, fig.cap="Fig 3: Proportion of variance explained. PRS calculated using linear combinations of PRS weights from LDpred-inf (PRS1, PRS2) and using local ancestry PRS (PRS3).HRS_AFR and PMBB_AFR"}
subplot(style(ggplotly(pD), showlegend=F), ggplotly(pE))
```
```{r, chunk='Fig4',out.height = "560px",out.width="960px",echo=FALSE, eval=T, fig.cap="Fig 4: Proportion of variance explained. PRS calculated using linear combinations of PRS weights from LDpred-inf (PRS1, PRS2) and using local ancestry PRS (PRS3).UKB_CHI"}
...
```{r, chunk='Fig3', out.height = "1260px",out.width="960px",echo=FALSE, eval=T, fig.cap="Fig 5: PRS for height for 2,503 individuals from the 1000 Genomes Project. PRS values are scaled around the mean and standard deviations of PRS for the EUR population, for each set of summary statistics."}
...
scale_fill_manual(values=cols3, name="Populations") + xlim(c(-10,10)) + ylim(c(0,1.2))  + theme_bw()
