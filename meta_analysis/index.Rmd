...
dt<-dt[!(ss1 %in% c("PRS_GIANT", "PRS_META_EUR", "PRS_META_ALL2", "PRS_META_EUREAS2"))]
...
cols<-unname(sample(unlist(wes_palettes),8))
shapes<-c(16,17,18,6,15)
dt<-dt %>% filter(LD_panel=='anc_specific') %>% dplyr::mutate(Test_Data=fct_reorder(Test_Data,pEUR), Label=fct_reorder(Label, N),Label2=fct_reorder(Label2,R2)) %>% as.data.table

p2<-ggplot(dt,aes(x=Test_Data, y=R2,color=Label, group=N, name=pEUR)) +
        #facet_grid(LD_panel~Label2) +
        facet_grid(~Label2) +
geom_point(aes(x=Test_Data, y=R2),alpha=0.8, size=2, position=position_dodge(width=0.8), show.legend = T) +
...
#cols<-c("#D8A499","#DD8D29","#899DA4","#5F5647","#A2A475","#7294D4","#550307", "#F1BB7B","#5BBCD6", "#B40F20")
cols<-unname(sample(unlist(wes_palettes),10))
...
res1<-readRDS('resPRS_multi_anc_specific.Rds')
...
expnd<-rbind(
        expand.grid(Test_Data="HRS_AFR", Set=c("UKBB","META_NEA", "META_ALL"), Label2=c("all", "pEUR<0.17", "pEUR>0.17"), PRS="PRS"), expand.grid(Test_Data="PMBB_AFR", Set=c("UKBB", "META_NEA", "META_ALL"), Label2=c("all", "pEUR<0.17", "pEUR>0.17"), PRS="PRS"),expand.grid(Test_Data="UKB_CHI", Set=c("UKBB", "BBJ", "UKBB+BBJ"), Label2="all", PRS="PRS"))
setDT(expnd)
...
expnd<-bind_rows(
        ...
        expnd %>% filter(Test_Data=="PMBB_AFR"& Set=='UKBB') %>% mutate(R2=ifelse(Label2=='all', 4.97, ifelse(Label2=="pEUR<0.17", 3.21, 6.88))) %>% as.data.table, 
        expnd %>% filter(Test_Data=="UKB_CHI"& Set=='UKBB') %>% mutate(R2=10.65) %>% as.data.table,
      ...
      #  expnd %>% filter(Test_Data=="PMBB_AFR"& Set=='META_EUR') %>% mutate(R2=ifelse(Label2=='all', 5.49, ifelse(Label2=="pEUR<0.17", 3.55,7.42))) %>% as.data.table, 
       # expnd %>% filter(Test_Data=="UKB_CHI"& Set=='META_EUR') %>% mutate(R2=12.21) %>% as.data.table,
        expnd %>% filter(Test_Data=="HRS_AFR"& Set=='META_ALL') %>% mutate(R2=ifelse(Label2=='all', 5.26, ifelse(Label2=="pEUR<0.17", 4.28,6.72))) %>% as.data.table,
         expnd %>% filter(Test_Data=="PMBB_AFR"& Set=='META_ALL') %>% mutate(R2=ifelse(Label2=='all', 6.05, ifelse(Label2=="pEUR<0.17", 4.60,9.45))) %>% as.data.table, 
        expnd %>% filter(Test_Data=="UKB_CHI"& Set=='META_ALL') %>% mutate(R2=8.36) %>% as.data.table,
        ...
        expnd %>% filter(Test_Data=="PMBB_AFR"& Set=='META_NEA') %>% mutate(R2=ifelse(Label2=='all', 5.42, ifelse(Label2=="pEUR<0.17", 4.82,5.93))) %>% as.data.table, 
        expnd %>% filter(Test_Data=="UKB_CHI"& Set=='BBJ') %>% mutate(R2=7.81) %>% as.data.table,
        expnd %>% filter(Test_Data=="UKB_CHI"& Set=='UKBB+BBJ') %>% mutate(R2=9.85) %>% as.data.table)
...
RES2t<-RES2[Test_Data=="HRS_AFR"][Set %in% c("UKBB--META_NEA", "META_ALL")] %>% mutate(Label2=fct_reorder(Label2,R2))
...
RES3t$Label2<-factor(RES3t$Label2,levels(RES3t$Label2)[c(1,3,2)])
pD<-ggplot() + 
        geom_line(data=RES2t, aes(x=Alpha, y=R2, color=Set, linetype=PRS), size=0.6, show.legend=F) + 
        geom_point(data= RES3t[Set!="META_ALL"], aes(x=Alpha, y=R2, group=Set), size=2, col=rep(cols[c(6,5)], each=3), alpha=0.8) + 
        facet_wrap(~Label2) +
        scale_color_manual(name="",values = cols2) + 
        scale_y_continuous(breaks = seq(0, 15, by = 1)) +
        geom_text(data=RES3t, aes(x=Alpha, y=R2, label=Set, group=Label2), size=2.5, nudge_x=c(rep(0.08,3), rep(-0.1,3), rep(0.5,3)), nudge_y=c(rep(-0.06,3), rep(-0.06,3), rep(0.2,3)))+
        theme_bw() + 
...
#
#
pD<-pD+ annotate("text", x = 0.1, y = 10, label = "HRS_AFR", size=3)
#

RES2t<-RES2[Test_Data=="PMBB_AFR"][Set %in% c("UKBB--META_NEA", "META_ALL")] %>% mutate(Label2=fct_reorder(Label2,R2))
...
#
#
pE<-pE+ annotate("text", x = 0.1, y = 10, label = "PMBB_AFR", size=3)
#
RES2t<-RES2[Test_Data=="UKB_CHI" & Label2=='all'][Set %in% c("UKBB--BBJ", "UKBB+BBJ")] %>% mutate(Label2=fct_reorder(Label2,R2))
...
pF<-ggplot() + 
        geom_line(data=RES2t, aes(x=Alpha, y=R2, color=Set, linetype=PRS), size=0.6, show.legend=F) + 
        geom_point(data= RES3t[Set!="UKBB+BBJ"], aes(x=Alpha, y=R2, group=Set), size=2, col=cols[c(6,4)], alpha=0.8) + 
        scale_color_manual(name="",values = cols2) + 
        scale_y_continuous(breaks = seq(0, 15, by = 1)) +
        geom_text(data=RES3t, aes(x=Alpha, y=R2, label=Set, group=Label2), size=2.5, nudge_x=c(0.04,-0.04,0.5), nudge_y=c(-0.05,-0.04,0.2))+
        theme_bw() + 
        labs(x="Alpha",y="Variance explained (%)", title="", legend="") + 
        theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12),axis.title.y=element_text(size=rel(1.4)), legend.title = element_blank())
#
...
```{r, chunk='Fig2',out.height = "960px",out.width="860px",echo=FALSE, eval=T, fig.cap="Fig 2: Proportion of variance explained. PRS calculated using linear combinations of PRS weights from LDpred-inf (PRS1, PRS2) and using local ancestry PRS (PRS3).HRS_AFR (top), PMBB_AFR (bottom)."}
subplot(style(ggplotly(pD), showlegend=F), ggplotly(pE), nrows=2, shareX=F, shareY=F)
```



```{r, chunk='Fig3',out.height = "960px",out.width="860px",echo=FALSE, eval=T, fig.cap="Fig 3: Proportion of variance explained. PRS calculated using linear combinations of PRS weights from LDpred-inf (PRS1, PRS2) and using local ancestry PRS (PRS3).UKB_CHI."}
ggplotly(pF)
```
...
```{r, chunk='Fig4', out.height = "1260px",out.width="1260px",echo=FALSE, eval=F, fig.cap="Fig 4: PRS for height for 2,503 individuals from the 1000 Genomes Project. PRS values are scaled around the mean and standard deviations of PRS for the EUR population, for each set of summary statistics.LD panel: 1000g_all"}
...
```{r, chunk='Fig4', out.height = "1260px",out.width="1260px",echo=FALSE, eval=T, fig.cap="Fig 4: PRS for height for 2,503 individuals from the 1000 Genomes Project. PRS values are scaled around the mean and standard deviations of PRS for the EUR population, for each set of summary statistics.LD panel: anc specific."}
set.seed(125)
cols3<-unname(sample(unlist(wesanderson::wes_palettes),5))
