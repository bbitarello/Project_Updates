...
```{r, chunk='R2_table', out.height = "460px",out.width="960px",echo=FALSE, eval=T, fig.cap="Fig 1: Proportion of variance explained. LDpred-Inf with sumamry statistics from UKBB, BBJ, meta-AFR, and meta-ALL. Points are empirical estimtes, and error bars are bootrstrap standard errors (n=5,000)."}
...
nams2<-names(res1)
res1<-lapply(1:5, function(x) as.data.table(as.data.frame(res1[[x]])))
res1<-lapply(res1, function(x) x[,Set:=nams])
res1<-lapply(1:5, function(x) res1[[x]][,Test_Data:=nams2[x]])
...
do.call(rbind, (res3 %>% group_by(Test_Data) %>% mutate(Test=Test_Data) %>% arrange(-value) %>% group_map(~head(.x, 5L))))
...
res4 %>% group_by(Test_Data) %>% mutate(Test=Test_Data) %>% arrange(-value) %>% group_map(~head(.x, 2L)) 
...
res5<-bind_rows(res3[Test_Data %in% c('PMBB_AFR', 'HRS_AFR', 'UKB_CHI')] %>% 
    filter(Set %in% unique(do.call(rbind, (res3 %>% filter(Test_Data %in% c('PMBB_AFR', 'HRS_AFR', 'UKB_CHI')) %>% group_by(Test_Data) %>% mutate(Test=Test_Data) %>% arrange(-value) %>% group_map(~head(.x, 4L))))$Set)) %>% mutate(Set=fct_reorder(Set, value)) %>% mutate(PRS="PRS1"), res4 %>% 
        filter(Set %in% unique(do.call(rbind, (res4 %>% group_by(Test_Data) %>% mutate(Test=Test_Data) %>% arrange(-value) %>% group_map(~head(.x, 4L))))$Set)) %>% mutate(Set=fct_reorder(Set, value)) %>% mutate(PRS="PRS2"))
#
#pC<-cowplot::plot_grid(pA,pB, labels=c("PRS1", "PRS2"), nrow=2)
pC<-ggplot(res5) + geom_line(aes(x=Alpha, y=value, color=Set, linetype=PRS)) + facet_wrap(~Test_Data, dir="h") +scale_color_manual(name='Summary Statistics',values = cols2[1:12])  + scale_y_continuous(breaks = seq(0, 20, by = 2)) + theme_bw()
ggplotly(pC)
```

```{r, chunk='R2_table', out.height = "460px",out.width="960px",echo=FALSE, eval=T, fig.cap="Fig 2: Venn Diagram"}
library(cowplot)
snp_IDS<-readRDS("all_snp_IDS.Rds")
sss<-gsub("PRS_", "",levels((dt %>% mutate(Summary_Stats=fct_reorder(Summary_Stats, N)))[,Summary_Stats]))
snp_IDS<-snp_IDS[sss]

v1<-ggvenn(snp_IDS[c(2,4,6,7)], fill_color = cols[c(2,4,6,7)], stroke_size = 0.5, set_name_size = 5)

ggplotly(v1)
v2<-ggvenn(snp_IDS[c(6,7,9)], fill_color = cols[c(6,7,9)], stroke_size = 0.5, set_name_size = 5)
ggplotly(v2)
v3<-ggvenn(snp_IDS[c(2,3,4,5)], fill_color = cols[c(2,3,4,5)], stroke_size = 0.5, set_name_size = 5)
ggplotly(v3)

v4<-ggvenn(snp_IDS[c(3,4,5)], fill_color = cols[c(3,4,5)], stroke_size = 0.5, set_name_size = 5)
#ggplotly(v4)
