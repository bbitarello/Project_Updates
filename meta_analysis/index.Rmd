...
```{r, chunk='Fig2', out.height = "560px",out.width="1160px",echo=FALSE, eval=T, fig.cap="Fig 2: Proportion of variance explained. PRS calculated using linear combinations of PRS weights from LDpred-inf (PRS1, PRS2) and using local ancestry PRS (PRS3)."}
set.seed(4194)
...
geom_line(aes(x=Alpha, y=value, color=Set, linetype=PRS), size=0.9, show.legend=F) + geom_point(data=expnd[Test_Data=="HRS_AFR"][1:3],aes(x=Alpha, y=value, color=Set), size=1.8, col=cols[c(7,5,9)]) +
scale_color_manual(name='',values = cols2[1:12]) + scale_y_continuous(breaks = seq(0, 10, by = 2)) +
theme_bw() + 
labs(x="Alpha",y="Variance explained (%)", title="HRS_AFR") + theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12), axis.title.y=element_text(size=rel(1.4)))+
         geom_text(data=expnd[Test_Data=="HRS_AFR"][1:3,], aes(x=Alpha, y=value, label=Set), nudge_x=c(0.05, -0.05, 0.05),nudge_y=c(0,-0.15, 0.4), check_overlap=T, size=3)

pE<-ggplot(res5[Test_Data=="PMBB_AFR"]) +
geom_line(aes(x=Alpha, y=value, color=Set, linetype=PRS), size=0.9) + 
        geom_point(data=expnd[Test_Data=="PMBB_AFR"][1:3],aes(x=Alpha, y=value, color=Set), size=1.8, col=cols[c(7,5,9)]) +
scale_color_manual(name='',values = cols2[1:12]) + scale_y_continuous(breaks = seq(0, 10, by = 2)) +
#facet_wrap(~Test_Data, dir="h") + 
theme_bw() + 
labs(x="Alpha",y="Variance explained (%)", title="PMBB_AFR") + theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12), axis.title.y=element_text(size=rel(1.4)))+
        geom_text(data=expnd[Test_Data=="PMBB_AFR"][1:3,], aes(x=Alpha, y=value, label=Set), nudge_x=c(0.05, -0.05, 0.05),nudge_y=c(0,-0.15, 0.4), check_overlap=T, size=3)


cowplot::plot_grid(pD,pE, nrow=1, labels=c("HRS_AFR", "PMBB_AFR"), rel_widths=c())
```
