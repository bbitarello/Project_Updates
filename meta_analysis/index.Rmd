...
```{r, chunk='Fig2', out.height = "460px",out.width="960px",echo=FALSE, eval=T, fig.cap="Fig 2: Proportion of variance explained. PRS calculated using linear combinations of PRS weights from LDpred-inf (PRS1, PRS2) and using local ancestry PRS (PRS3)."}
set.seed(4194)
...
res5<-bind_rows(res3[Test_Data %in% c('PMBB_AFR', 'HRS_AFR')] %>% mutate(Set=fct_reorder(Set, value)) %>% mutate(PRS="PRS1"), res4[Test_Data %in% c('PMBB_AFR', 'HRS_AFR')] %>%  mutate(Set=fct_reorder(Set, value)) %>% mutate(PRS="PRS2")) %>% filter(Set %in% "UKBB--META_NEA") %>% as.data.table
...
pC<-ggplot(res5) + geom_line(aes(x=Alpha, y=value, color=Set, linetype=PRS)) + facet_wrap(~Test_Data, dir="h") +scale_color_manual(name='Summary Statistics',values = cols2[1:12])  + scale_y_continuous(breaks = seq(0, 10, by = 2)) + theme_bw() + labs(x="",y="Variance explained (%)", title="") + theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12), axis.title.y=element_text(size=rel(1.4)))
ggplotly(pC)
```
