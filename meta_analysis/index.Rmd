...
#dt[, Label:=ifelse(ss1=='PRS_META_EUREAS', 'UKBB+BBJ', ifelse(ss1=='PRS_META_EUREAS2', 'UKBB+GIANT+BBJ', ifelse(ss1=='PRS_META_AFR2', 'META_AFR+PAGE', ifelse(ss1=='PRS_META_ALL2','UKBB+GIANT+BBJ+PAGE+META_AFR', ifelse(ss1=='PRS_META_ALL', 'UKBB+BBJ+PAGE+META_AFR',ifelse(ss1=='PRS_META_EUR', 'UKBB+GIANT',ss1))))))]
...
cols<-unname(sample(unlist(wes_palettes),12))
...
RES2t<-RES2[Test_Data=="HRS_AFR"][Set %in% c("UKBB--META_NEA", "META_ALL2")] %>% mutate(Label2=fct_reorder(Label2,R2))
...
pD<-ggplot() + 
        ...
        geom_text(data=RES3t, aes(x=Alpha, y=R2, label=Set, group=Label2), size=2.5, nudge_x=c(rep(0.1,2), rep(0.1,2), rep(-0.1,3),rep(0.15,2)), nudge_y=c(rep(-0.1,2), rep(0.1,2), rep(-0.1,2),rep(0.2,2)))+
        facet_wrap(~Label2) +
        theme_bw() + 
...
RES2t<-RES2[Test_Data=="PMBB_AFR"][Set %in% c("UKBB--META_NEA", "META_ALL2")] %>% mutate(Label2=fct_reorder(Label2,R2))
...
pE<-ggplot() + 
        ...
        scale_color_manual(name="",values = cols2) + 
        scale_y_continuous(breaks = seq(0, 15, by = 1)) +
        geom_text(data=RES3t, aes(x=Alpha, y=R2, label=Set, group=Label2), size=2.5, nudge_x=c(rep(0.1,2), rep(0.1,2), rep(-0.1,2),rep(0.15, 2)), nudge_y=c(rep(-0.1,2), rep(0.1,2), rep(-0.1,2),rep(0.2,2)))+
        facet_wrap(~Label2) +
        theme_bw() + 
        labs(x="Alpha",y="Variance explained (%)", title="", legend="") + 
        theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12),axis.title.y=element_text(size=rel(1.4)), legend.title = element_blank())
...
#
RES2t<-RES2[Test_Data=="UKB_CHI" & Label2=='all'][Set %in% c("UKBB--BBJ", "META_ALL2")] %>% mutate(Label2=fct_reorder(Label2,R2))
