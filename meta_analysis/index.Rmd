...
p1000<-ggplot(g1000, aes(x=PRS_scaled, fill=super_pop))+ facet_wrap(~Label,labeller = label_wrap_gen(width=2))+
geom_density(alpha=0.6) + labs(x="Density",x="Scaled Polygenic Score", title="1000 Genomes")+
theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12), axis.title.y=element_text(size=rel(1.4)))+
scale_fill_manual(values=cols3, name="Populations")
```

```{r, chunk='Fig4', out.height = "660px",out.width="960px",echo=FALSE, eval=T, fig.cap="Fig 4: 1000 GENOMES"}
ggplotly(p1000)
```
