...
dt[,N:=ifelse(Summary_Stats=='PRS_UKBB',360388, ifelse(Summary_Stats=='PRS_BBJ',159095, ifelse(Summary_Stats=='PRS_PAGE',49781, ifelse(Summary_Stats=='PRS_GIANT', "50003-253280", ifelse(Summary_Stats=='PRS_META_AFR', 41109, ifelse(Summary_Stats=='PRS_META_AFR2',"84334-90890", ifelse(Summary_Stats=='PRS_META_NEA', "208876-249985", ifelse(Summary_Stats=="PRS_META_EUR", "410391-613667", ifelse(Summary_Stats=="PRS_META_ALL", "410169-610373", "660001-660019")))))))))]
dt$Summary_Stats<-factor(dt$Summary_Stats)
library(forcats)
p2<-dt %>% mutate(Test = fct_reorder(Test_Data,pEUR)) %>% mutate(Summary_Stats=fct_reorder(Summary_Stats, N)) %>%
ggplot(aes(x=Test, y=R2,color=Summary_Stats, Group=pEUR)) +
geom_point(size=4, alpha=0.7, width=0.4, position=position_dodge(width=0.5)) + 
geom_errorbar(aes(ymin=CI_L, ymax=CI_U), width=0.1, linetype = "dotted",position=position_dodge(width=0.5)) +        
scale_color_manual(name='',values = c(wes_palette("Royal2",5),wes_palette("Darjeeling1",5))) + 
theme_bw()+ labs(x="Test Dataset",y="Variance explained (%)", title="", cex=3)
ggplotly(p2)
