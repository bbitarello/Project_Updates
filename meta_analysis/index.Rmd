...
pD<-ggplot() + 
        ...
        geom_text(data=RES3t, aes(x=Alpha, y=R2, label=Set, group=Label2), size=2.5, nudge_x=c(rep(0.08,3), rep(-0.1,3), rep(0.5,3)), nudge_y=c(rep(-0.05,3), rep(-0.05,3), rep(0.2,3)))+
        theme_bw() + 
...
pF<-ggplot() + 
        ...
        scale_color_manual(name="",values = cols2) + 
        scale_y_continuous(breaks = seq(0, 15, by = 1)) +
        geom_text(data=RES3t, aes(x=Alpha, y=R2, label=Set, group=Label2), size=2.5, nudge_x=c(0.04,-0.05,0.5), nudge_y=c(-0.05,-0.05,0.2))+
        theme_bw() + 
        labs(x="Alpha",y="Variance explained (%)", title="", legend="") + 
        theme(axis.text.x = element_text(size=12), axis.text.y = element_text(size=12),axis.title.y=element_text(size=rel(1.4)), legend.title = element_blank())
#
